...
위의 메소드 removeIf의 매개변수 선언은 다음과 같다.
...
보충을 해서 의역을 하면, 컬렉션 인스턴스에 저장된 인스턴스를 다음 test 메소드의 인자로 전달했을 때, true가 반환되는 인스턴스는 모두 삭제하겠다는 뜻이다.
...
이를 List<Integer> 인스턴스, 그리고 List<Double> 인스턴스의 removeIf 메소드에 인자로 전달을 하였다. 이것이 가능한 이유는 removeIf 메소드의 매개변수 선언이 다음과 같기 때문이다.
