...
/*
	...
	앞서 최종 연산 sum과 forEach를 사용해 보았다. 이에 이어서 최종 연산을 대표한느 reduce 메소드를
	소개하고자 한다.

	리덕션과 reduce 메소드

	'리덕션(Reduction)'은 데이터를 축소하는 연산'을 뜻한다. 앞서 보인 sum도 리덕션 연산에 해당한다.
	다수의 데이터를 더하여 '합'이라는 하나의 데어터만 남겼으니 sum도 리덕션 연산이다. 이렇듯 대부분의
	리덕션 연산은 사용하기 쉬운 편이다. 그러나 여기 조금 다른 리덕션을 진행하는 메소드가 있어
	소개하고자 한다.
	...
	위 그림은 스트림에 총 4개의 데이터가 존재한느 상황을 표현하였다. 이 상황에서 reduce는 내부적으로 apply를
	...
	인다로 두 개의 문자열을 전달받아서 그 길이를 비교하여 긴 문자열을 반환하는 람다식이다.(두 문자열의 길이가
	동일할 땐느 두 번째 문자열을 반환하도록 작성하였다.) 그리고 이 람다식을 다음과 같이 reduce 메소드를
	호출하면서 인자로 전달하였다.

	String str = ls.stream()
					.reduce("", lc);	// 스트림이 빈 경우 빈 문자열 반환

	따라서 reduce는 내부적으로 스트림을 구성하는 문자열의 길이를 비교해 나간다. 그리고 마지막에는
	가장 긴 문자열을 반환하게 된다.
	...
	비어 있지 않은 경우엔느 이를 스트림의 첫 번째 데이터로 간주하고 리덕션을 진행한다. 때문에 "Empty Stream"이
