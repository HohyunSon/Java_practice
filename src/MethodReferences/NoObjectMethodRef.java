...
메소드 참조를 위한 IBox::larger만 보면 static 메소드 참조처럼 보인다. 그러나 이는 인스턴스
메소드의 참조이다.
...
실행된다. (아래의 코드는 메소드 정의가 아니다. applyAsInt 메소드가 호출될 때 실행되는 내용을
표현한 것이다.)
...
수정한 겨로가를 보일 차례인데, 차이가 나는 것은 다음 문장이 전부이니 위 예제를 직접 수정하여 그 결과를
확인하도록 하자.
