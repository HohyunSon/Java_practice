...
람다식을 작성하다 보면 인스턴스를 생성하고 이의 참조 값을 반환해야 하는 경우가 있다. 그리고 이러한
경우에도 '메소드 참조' 방식을 쓸 수 있다.
...
정리하면, 람다식을 이루는 문장이 '단순히 인스턴스의 생성 및 참조 값의 반환'일 경우, 이를 다음
형태의 메소드 참조로 바꿀 수 있다.

ClassName::new

이번에도 이러한 줄임이 가능한 이유를 역으로 생각해보기 ㅜ이해 다음 코드를 보자.
...
참조변수 f의 자료형이 Function<char[], String>이므로 매개변수 형이 char[]인 다음
생성자를 참조하게 된다.
...
다음 내용으로 apply 메소드가 호출된다. (아래 코드는 메소드 정의가 아니다. apply가
호출될 때 실행되는 내용을 표현한 것이다.) 그리고 다시 한번 말하지만 이는 어디까지나 약속이다.
