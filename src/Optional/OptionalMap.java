...
map은 제네릭 클래스에 정의된 제네릭 메소드임을 알 수 있따.(T는 제네릭 클래스의 멤버임을 알려주고,
U는 제네릭 메소드임을 알려준다.)
...
구현에 대한 람다식을 작성하면 된다. 즉 T는 Optional<String> 인스턴스 생성 시 String으로
이미 결정이 난 상태이다.

U apply(String t)

그리고 map 메소드는 <U>에 대한 제네릭 메소드이므로, 이 U는 메소드를 호출하는 시점에서 결정이 된다.
그렇다면 map 메소드가 하는 일은 무엇일까? (사실 이 내요이 제일 중요하다.)
...
위 문장의 람다식은 String 인스턴스의 참조 값을 반환한다. 따라서 위 문장의 map이 호추로디는 순간
반환형 U가 String으로 결저오디어 위의 람다식은 다음 apply 메소드의 몸체를 구성하게 된다. 그리고
위 문장의 map이 호출되면 아래의 apply 메소드의 인자로는 참조변수 os1이 지니는 인스턴스가 전달이
된다.
...
그리고 위 메소드가 호출되었을  때 반환하는 값을 map은 그냥 반환하지 않고 Optional 인스턴스로
감싸서 반환한다.

[참고] 제네릭 클래스에 정의된 제네릭 메소드

Optional<T>는 제레릭 클래스이다. 따라서 Optional<String>의 인스턴스 메소드인 map은 다음과
같은 상황이다.
