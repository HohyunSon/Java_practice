...
class ContInfo0 {
    ...
    public ContInfo0(Optional<String> ph, Optional<String> ad) {
        phone = ph;
        adrs = ad;
    }
...
class FlatMapElseOptional {
    public static void main(String[] args) {
        Optional<ContInfo0> ci = Optional.of(
                new ContInfo0(Optional.ofNullable(null), Optional.of("Republic of Korea"))
        );
...
위 예제는 MapElseOptional.java를 flatMap 메소드를 사용하는 형태로 수정한 결과이다. 먼저 다음
클래스 정의를 보자.
...
Optional로 감싸서 반환하는 map 메소드이 특성상 다음과 같이 get 메소드 호출을 통해서 꺼내는 과정을
거쳐야 하기 때문이다.
