...
두 메소드 of와 ofNullable의 차이점은 null의 허용 여부에 있다. ofNullable의 인자로는 null을
전달할 수 있다. 즉 비어 있는 Optional 인스턴스를 생성할 수 있다. 반면 of 메소드에는 null을
인자로 전달할 수 없다. null을 전달할 경우 NullPointerException이 발생한다. 그리고 Optional
인스턴스르 대상으로 다음과 같이 내용물의 존재 여불르 확인할 수 있고, 또 해당 내용물을 꺼낼 수도 있다.

if(os1.isPresent())		// 내용물 존재하면 isPresent는 true 반환
	System.out.println(os1.get());	// get을 통한 내용물 반환

그런데 위의 예제에서는 Optional의 매력을 찾을 수 없다. 그러나 다음 예제에서는 Optional의
매력을 조금 보여준다.(StringOptional2.java)
