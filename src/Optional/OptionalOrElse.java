...
생서오디어 반환된다.(그 아래의 ofNullable 메소드의 호출 문장과 동일한 문장이다.)

Optional<String> os1 = Optional.empty();
	-> Optional<String> os1 = Optional.ofNullable(null);
그리고 이 참조변수를 대상을 ㅗ다음 문장을 실행하면 map 메소드가 호출되고, 이어서 orElse
메소드가 호출되어 그 반환 값이 s1에 저장된다.

String s1 = os1.map(s->s.toString()).orElse("Empty");

그런데 os1이 참조하는 Optional 인스턴스는 비어 있다. 이러한 경우 map은 빈 Optional
인스턴스를 생성하여 반환한다. 결국 위의 문장은 map이 반환한 빈 Optional 인스턴스를 대상으로
orElse메소드를 호출하게 된다. 그리고 이렇듯 빈 Optional 인스턴스를 대상으로 orElse
메소들를 호출하면, orElse를 호출하면서 전달된 인스턴스가 대신 반환된다. 즉 위의 문장이 실행되면
s1은 문자열 "Empty"를 참조하게 된다.
